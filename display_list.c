...
** bonus: on columns
**
** padding - done according to the longest file name (maxlen.x)
** maxlen.y - used for -s option to calculate the width of blocks display
...
void		display_name(t_file *l, int flags, int blocks_len)
{
	...
	{
		if (S_ISDIR(l->mode))
			ft_printf("\033[1;36m\%s\033[0m", l->name);
		else
			ft_printf((S_IXUSR & l->mode) ? "\033[31m%s\033[0m" : \
					"%s", l->name);
	}
	else
		ft_printf("%s", l->name);
